                            ...
                            abi_long arg8)
{
	...
	switch (num) {
		case 3://mmap
			fprintf(stderr, "%s: mmap fd=%08x addr=%08x len=%08x prot=%08x flags=%08x offset=%08x\n",
		...
		case 5://epoll_ctl;
			...
			if (get_user_u64(tmp, arg4 + 4)) {
				fprintf(stderr, "%s: failed to read epoll_ctl data\n",
						__func__);
			}
			else {
				fprintf(stderr, "%s: epoll data=%016lx\n", __func__, tmp);
				if (tmp) {
					epoll_data = tmp;
				}
			}
		...
		case 15://ioctl
			...
			if (arg1 == TEEGRIS_FAKE_FD(TEEGRIS_FAKE_FILE_DEV_CRYPTO)) {
				...
				if (arg2 == 7)
				{
					...
					fprintf(stderr, "%s: dev crypto ioctl 7 req=%08x cookie=%08x\n",
							__func__, v0, cookie);
...
#define RECVMSG_IOV_OFF 0x28
			...
			fprintf(stderr, "%s: recvmsg in iov_len=%08x\n",
					__func__, t32);

			{
				...
				if (!recv_type) {
					...
					if (!get_user_u64(tmp, arg2)) {
						fprintf(stderr, "%s: recvmsg ptr %08lx\n", __func__, tmp);
		...
		case 90://fstat
			{
				...
				if (!is_error(ret)) {
					...
					if (!lock_user_struct(VERIFY_WRITE, target_st, arg2, 0))
					{
						fprintf(stderr, "%s: failed to lock struct for stat\n", __func__);
						ret = -TARGET_EFAULT;
						goto done;
					}
